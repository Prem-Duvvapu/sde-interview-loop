...
/**
 * Loads behavioral questions from {@code question-bank/behavioral/*.yaml} at startup.
 * Same fail-fast, random-among-matching-difficulty pattern as
 * {@code content.dsa.DsaQuestionBank} — kept independent rather than shared, deliberately.
 */
@Component
public class BehavioralQuestionBank {

    private static final Logger log = LoggerFactory.getLogger(BehavioralQuestionBank.class);
    ...
    private final Map<String, BehavioralQuestion> bySlug = new LinkedHashMap<>();
    private final Map<String, String> contentHashes = new HashMap<>();

    @PostConstruct
    public void init() {
        Path dir = Paths.get(bankDir, "behavioral");
        if (!Files.isDirectory(dir)) {
            throw new IllegalStateException("Behavioral question bank directory not found: " + dir.toAbsolutePath());
        }
        ...
        try (DirectoryStream<Path> stream = Files.newDirectoryStream(dir, "*.yaml")) {
            for (Path file : stream) {
                ...
                try {
                    ...
                    BehavioralQuestion q = yamlMapper.readValue(raw, BehavioralQuestion.class);
                    validate(stem, q);
                    bySlug.put(q.getSlug(), q);
                    contentHashes.put(q.getSlug(), sha256(q.getPrompt()));
                } catch (Exception e) {
        ...
        if (!errors.isEmpty()) {
            StringBuilder sb = new StringBuilder("Behavioral question bank validation failed:\n");
            errors.forEach(err -> sb.append("  - ").append(err).append("\n"));
            throw new IllegalStateException(sb.toString());
        }
        if (bySlug.isEmpty()) {
            throw new IllegalStateException("No behavioral questions found in " + dir.toAbsolutePath());
        }

        log.info("Loaded {} behavioral question(s): {}", bySlug.size(), bySlug.keySet());
    }

    private void validate(String stem, BehavioralQuestion q) {
    ...
    public BehavioralQuestion get(String slug) {
        BehavioralQuestion q = bySlug.get(slug);
        if (q == null) {
            throw new NoSuchElementException("No behavioral question with slug: " + slug);
        }
        return q;
    }
    ...
    public BehavioralQuestion selectFor(String difficultyTarget) {
        String target = difficultyTarget == null || difficultyTarget.isBlank() ? "medium" : difficultyTarget;
        List<BehavioralQuestion> exact = bySlug.values().stream()
        ...
        for (int i : byDistance) {
            ...
            List<BehavioralQuestion> matches = bySlug.values().stream()
                    .filter(q -> q.getDifficulty().equals(candidate))
                    .toList();
            if (!matches.isEmpty()) {
                log.warn("No behavioral question at difficulty '{}', falling back to '{}'", target, candidate);
                return matches.get(ThreadLocalRandom.current().nextInt(matches.size()));
            }
        }
        throw new IllegalStateException("Behavioral question bank is empty");
    }
