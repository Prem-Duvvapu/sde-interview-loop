...
/**
 * Loads LLD questions from {@code question-bank/lld/*.yaml} at startup. Same fail-fast,
 * random-among-matching-difficulty pattern as
 * {@link com.premd.interviewloop.content.dsa.DsaQuestionBank} — kept as a separate class
 * rather than shared with it, deliberately (see {@link LldQuestion}).
 */
@Component
public class LldQuestionBank {

    private static final Logger log = LoggerFactory.getLogger(LldQuestionBank.class);
    ...
    private final Map<String, LldQuestion> bySlug = new LinkedHashMap<>();
    private final Map<String, String> contentHashes = new HashMap<>();

    @PostConstruct
    public void init() {
        Path dir = Paths.get(bankDir, "lld");
        if (!Files.isDirectory(dir)) {
            throw new IllegalStateException("LLD question bank directory not found: " + dir.toAbsolutePath());
        }
        ...
        try (DirectoryStream<Path> stream = Files.newDirectoryStream(dir, "*.yaml")) {
            for (Path file : stream) {
                ...
                try {
                    ...
                    LldQuestion q = yamlMapper.readValue(raw, LldQuestion.class);
                    validate(stem, q);
        ...
        if (!errors.isEmpty()) {
            StringBuilder sb = new StringBuilder("LLD question bank validation failed:\n");
            errors.forEach(err -> sb.append("  - ").append(err).append("\n"));
            throw new IllegalStateException(sb.toString());
        }
        if (bySlug.isEmpty()) {
            throw new IllegalStateException("No LLD questions found in " + dir.toAbsolutePath());
        }

        log.info("Loaded {} LLD question(s): {}", bySlug.size(), bySlug.keySet());
    }

    private void validate(String stem, LldQuestion q) {
    ...
    public LldQuestion get(String slug) {
        LldQuestion q = bySlug.get(slug);
        if (q == null) {
            throw new NoSuchElementException("No LLD question with slug: " + slug);
        }
        return q;
    }
     ...
     * questions if there's no exact match — the bank currently skews medium+ by design (a
     * "soft" LLD round is not useful practice), so an "easy" request routinely falls back.
     */
    public LldQuestion selectFor(String difficultyTarget) {
        String target = difficultyTarget == null || difficultyTarget.isBlank() ? "medium" : difficultyTarget;
        List<LldQuestion> exact = bySlug.values().stream()
        ...
        for (int i : byDistance) {
            ...
            List<LldQuestion> matches = bySlug.values().stream()
                    .filter(q -> q.getDifficulty().equals(candidate))
                    .toList();
            if (!matches.isEmpty()) {
                log.warn("No LLD question at difficulty '{}', falling back to '{}'", target, candidate);
                return matches.get(ThreadLocalRandom.current().nextInt(matches.size()));
            }
        }
        throw new IllegalStateException("LLD question bank is empty");
    }
