...
/**
 * Loads HLD questions from {@code question-bank/hld/*.yaml} at startup. Same fail-fast,
 * random-among-matching-difficulty pattern as the DSA and LLD banks — kept as a separate
 * class rather than shared with them, deliberately (see {@link HldQuestion}).
 */
@Component
public class HldQuestionBank {

    private static final Logger log = LoggerFactory.getLogger(HldQuestionBank.class);
    ...
    private final Map<String, HldQuestion> bySlug = new LinkedHashMap<>();
    private final Map<String, String> contentHashes = new HashMap<>();

    @PostConstruct
    public void init() {
        Path dir = Paths.get(bankDir, "hld");
        if (!Files.isDirectory(dir)) {
            throw new IllegalStateException("HLD question bank directory not found: " + dir.toAbsolutePath());
        }
        ...
        try (DirectoryStream<Path> stream = Files.newDirectoryStream(dir, "*.yaml")) {
            for (Path file : stream) {
                ...
                try {
                    ...
                    HldQuestion q = yamlMapper.readValue(raw, HldQuestion.class);
                    validate(stem, q);
        ...
        if (!errors.isEmpty()) {
            StringBuilder sb = new StringBuilder("HLD question bank validation failed:\n");
            errors.forEach(err -> sb.append("  - ").append(err).append("\n"));
            throw new IllegalStateException(sb.toString());
        }
        if (bySlug.isEmpty()) {
            throw new IllegalStateException("No HLD questions found in " + dir.toAbsolutePath());
        }

        log.info("Loaded {} HLD question(s): {}", bySlug.size(), bySlug.keySet());
    }

    private void validate(String stem, HldQuestion q) {
    ...
    public HldQuestion get(String slug) {
        HldQuestion q = bySlug.get(slug);
        if (q == null) {
            throw new NoSuchElementException("No HLD question with slug: " + slug);
        }
        return q;
    }
     ...
     * questions if there's no exact match — mirrors {@code LldQuestionBank.selectFor}.
     */
    public HldQuestion selectFor(String difficultyTarget) {
        String target = difficultyTarget == null || difficultyTarget.isBlank() ? "medium" : difficultyTarget;
        List<HldQuestion> exact = bySlug.values().stream()
        ...
        for (int i : byDistance) {
            ...
            List<HldQuestion> matches = bySlug.values().stream()
                    .filter(q -> q.getDifficulty().equals(candidate))
                    .toList();
            if (!matches.isEmpty()) {
                log.warn("No HLD question at difficulty '{}', falling back to '{}'", target, candidate);
                return matches.get(ThreadLocalRandom.current().nextInt(matches.size()));
            }
        }
        throw new IllegalStateException("HLD question bank is empty");
    }
